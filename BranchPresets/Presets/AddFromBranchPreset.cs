...
namespace BranchPresets.Presets
{
    /// <summary>
    ...
    public class AddFromBranchPreset : AddFromTemplateProcessor
    {
        ...
        protected virtual void RewriteBranchRenderingDataSources(Item item, BranchItem branchTemplateItem, string branchRoot)
        {
            ...
            Helpers.Layout.LayoutHelper.ApplyActionToAllRenderings(item, rendering =>
            {
                if (string.IsNullOrWhiteSpace(rendering.Datasource))
                {
                    return Helpers.Layout.RenderingActionResult.None;
                }
                ...
                // if there was no valid target item OR the target item is not a child of the branch template we skip it
                ...
                // if the target item was a valid item under branch, but the same relative path does not exist under the branch instance
            ...
            item.Children.ToList().ForEach(x => this.RewriteBranchRenderingDataSources(x, branchTemplateItem, branchRoot));
        }
    }
}